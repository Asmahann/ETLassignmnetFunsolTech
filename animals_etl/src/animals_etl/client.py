...
class AnimalsClient:
        ...
        while True:
            ...
            try:
                ...
                if attempt > self.max_retries:
                    logger.error("Giving up after %s attempts to %s %s", attempt, method, url)
                    raise
                logger.warning("Error on %s %s attempt %s: %s", method, url, attempt, exc)
                await asyncio.sleep(backoff)
                backoff *= 2

    async def list_animals_page(self, page: int) -> List[AnimalSummary]:
        response = await self._request_with_retries("GET", f"/animals/v1/animals?page={page}")
        data = response.json()
    ...
    async def fetch_animal_detail(self, animal_id: int) -> AnimalDetail:
        response = await self._request_with_retries("GET", f"/animals/v1/animals/{animal_id}")
        data = response.json()
    ...
    async def post_home_batch(self, animals: Sequence[AnimalDetail]) -> None:
        ...
        await self._request_with_retries("POST", "/animals/v1/home", json=payload)
