...
class AnimalDetail(BaseModel):
    ...
    def parse_born_at(cls, value: object) -> Optional[datetime]:
        ...
        if isinstance(value, (int, float)):
            ...
            # Some records use millisecond timestamps that can overflow seconds-based parsing.
            for divisor in (1.0, 1000.0):
                try:
                    return datetime.fromtimestamp(timestamp / divisor, tz=timezone.utc)
                except (OverflowError, OSError, ValueError):
                    continue
            # If we still cannot parse, drop the value instead of failing the whole record.
            return None
        if isinstance(value, str):
